...
Coverity wird nur bei Commits auf den Coverity Branch ausgeführt, da die Nutzung von Coverity beschränkt ist. Dies wir durchgeführt, bevor in den Master Branch gemerged wird. Die Anzahl der Builds pro Woche ist auf 28 mit maximal 4 Tests am Tag beschränkt, wenn weniger als 100.000 Zeilen Code getestet werden. Wurden die maximale Anzahl der Builds pro Tag erreicht, werden weitere Builds an dem entsprechenden Tag abgelehnt. Wenn Travis CI oder Coverity Fehler erzeugen, müssen diese von demjenigen, der gepusht hat, zum nächstmöglichen Zeitpunkt behoben werden. 

\newpage

\section{Modularität und Erweiterbarkeit}
...
Unser Projekt gliedert sich in drei Hauptmodule: Das Hauptprogramm \mbox{(Modul 1)}, die Anbindung an Nightscout und der Daten-Parser \mbox{(Modul 2)} und die Repräsentation der Daten \mbox{(Modul 3)}. Diese Module können unabhängig voneinander verwendet werden, wobei sie jeweils auf das in der obigen Liste folgende Modul aufbauen. Unser Auftraggeber verwendet zum Beispiel die im dritten Modul definierten Datenklassen auch für andere Projekte (diese Klassen wurden uns vorgegeben), und die Anbindung an Nightscout kann benutzt werden, um beliebige Daten von und zu Nightscout zu übertragen.

Das Hauptprogramm gliedert sich in vier weitere Module: Die Hauptklasse und ihre dazugehörigen Klassen, zum Starten des Programms \mbox{(Modul 1)}, die Datenquellen, zum Einlesen und Abspeichern der Daten \mbox{(Modul 2)}, und die Algorithmen, zur Verarbeitung der Daten \mbox{(Modul 3)}. Häufig benutzte Funktionen und die Implementierungen der verwendeten mathematischen Modelle, welche in verschiedenen Algorithmen benötigt werden, werden in eigene Klassen ausgelagert \mbox{(Modul 4)}. Dies ermöglicht es viele Funktionen in verschiedenen Algorithmen wieder zu verwenden und diese unabhängig von den Algorithmen testen zu können.

In einem beispielhaften Programmablauf wird also das Hauptprogramm gestartet und öffnet, je nach Argumenten und Konfiguration, eine Datenquelle, um die dort hinterlegten Daten an einen Algorithmus weiterzugeben. Dieser benutzt dann gegebenenfalls verschiedene der implementierten mathematischen Modelle und gibt am Ende neue, berechnete Daten an das Hauptprogramm zurück.
...
Bei der Kontrolle von Pull-Requests auf den Master Branch achtet die kontrollierende Person darauf, dass die verwendeten Entwurfsmuster korrekt umgesetzt wurden, und zum Beispiel ein Algorithmus nicht direkt Daten an das Hauptprogramm weiter gibt sondern dies nur über das dafür vorgesehene Strategie Interface passiert. Außerdem wird darauf geachtet neue Funktionen so abstrakt wie möglich zu implementieren, um die Wiederverwendung an anderen Stellen zu erleichtern. Dabei wird kontrolliert, ob diese neuen Funktionen sinnvoll in ihrem Modul platziert sind, oder ob eine Implementation in einem anderen Modul die Wiederverwendung an mehr Stellen ermöglichen würde. Die im Anhang enthaltene Checkliste beinhaltet auch die hierfür wichtigen Punkte.
