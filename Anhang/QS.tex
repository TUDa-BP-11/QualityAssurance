...
Die Konfiguration von \href{https://scan.coverity.com/projects/tuda-bp-11-opendiabetes-uam-heuristik}{Coverity Scan} erwies sich als schwierig, man konnte zwar Pfade angeben die nicht getestet werden sollen aber niemand von uns konnte die Einstellungen speichern. Andere Benutzer hatten das gleiche Problem und wir sind dazu übergegangen die Defekte aus dem Code den unser Auftrageber als "intentional" zu markieren. Defekte in den Tests haben wir dann dennoch angeschaut, sie konnten aber in allen Fällen als "false positiv" oder "intentional" markiert werden.\\
Die häufigsten "False Positivs" waren String to Byte bzw. Byte to String Konvertierungen.
Die meisten Defekte die gefunden wurden waren Variablen und Felder die nicht benutzt oder nur aktualisiert wurden, danach waren es Felder und Methoden die nicht als statisch markiert wurden obwohl sie so benutzt wurden. 

\subsection{Bsp. gefundener Bug}
In diesem Beispiel gibt die Funktion getRawBasalTreatments() null zurück und löst damit eine NullPointerException aus, da sie sehr kurz vor einem Scan erstellt wurde ist es keinem von uns vorher aufgefallen. 
Bild einfügen!!!

\subsection{Bsp. False Positive}
Hier wird !allFilesSet(config) zu false ausgewertet.
Bild einfügen!!!

Weiter unten wird das Inverse ,also allFilesSet(config) auch zu false ausgewertet.
Bild einfügen!!!
\end{document}
